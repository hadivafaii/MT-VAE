...
class ReadoutTrainer(BaseTrainer):
    def __init__(self,
        ...
        self.core = kwargs['core'].to(self.device).eval()
    ...
    def validate(self, global_step: int = None, verbose: bool = True, **kwargs):
        self.model.eval()
        ...
        loss = np.mean(loss_list) / self.dl_valid.batch_size
        ...
        if global_step is not None:
            ...
            self.writer.add_scalar("extras/mean_nnll", np.mean(nnll), global_step)
            self.writer.add_scalar("extras/median_nnll", np.median(nnll), global_step)
            self.writer.add_scalar("extras/mean_r2", np.mean(r2), global_step)
            self.writer.add_scalar("extras/median_r2", np.median(r2), global_step)
        if verbose:
    ...
    def setup_data(self):
        dl_train, dl_valid = create_readout_dataset(self.config, self.train_config)
